...
%define stable_update 223
# Set rpm version accordingly
%if 0%{?stable_update}
%define stablerev %{stable_update}
%define stable_base %{stable_update}
%endif
%define specrpmversion 5.%{base_sublevel}.%{stable_update}
...
%define gitrev 0
# Set rpm version accordingly
%define specrpmversion 5.%{upstream_sublevel}.0
%endif
...
# Source300: kernel-abi-whitelists-%{specrpmversion}-%{distro_build}.tar.bz2
# Source301: kernel-kabi-dw-%{specrpmversion}-%{distro_build}.tar.bz2
...
# Patch1: patch-%{specrpmversion}-redhat.patch
...
# Does not apply to 5.10.103 anymore. As it's for aarch64 we just ignore it.
...
# Does not apply to 5.10.103 anymore. As it's for ARM we just ignore it.
...
# Does not apply to 5.10.146 anymore. As it's for Allwinner Sun6i we just ignore it.
...
# Does not apply to 5.10.103 anymore. As it's for arm64 we just ignore it.
...
# libbpf: stop using feature-detection Makefiles (due to false negatives)
Patch999: bpf-next-libbpf-stop-using-feature-detection-Makefiles.diff
...
%if "0%{?variant}"
Obsoletes: kernel-headers < %{specrpmversion}-%{pkg_release}
Provides: kernel-headers = %{specrpmversion}-%{pkg_release}
%endif
%description headers
...
# LF: For the life of me I couldn't get the %py3_shebang_fix macro to work, I must be missing something about RPM spec files.
# LF: Hence this ugly hack... 
python3 -B /usr/lib/rpm/redhat/pathfix.py -i /usr/bin/python3 -n -p -k \
...
%if %{with_configchecks}
	OPTS="$OPTS -w -n -c"
%endif
./process_configs.sh $OPTS kernel %{specrpmversion}
...
%if %{with_doc}
docdir=$RPM_BUILD_ROOT%{_datadir}/doc/kernel-doc-%{specrpmversion}
...
%files doc
%defattr(-,root,root)
%{_datadir}/doc/kernel-doc-%{specrpmversion}/Documentation/*
%dir %{_datadir}/doc/kernel-doc-%{specrpmversion}/Documentation
%dir %{_datadir}/doc/kernel-doc-%{specrpmversion}
%endif
